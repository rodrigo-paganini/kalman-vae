...
class KalmanFilter(nn.Module):
    ...
    def filter_step(self, mu_t_t, Sigma_t_t, y_t, u_t):
        ...
        Sigma_t_t = Sigma_t_tprev - K @ PCT.mT 
    ...
    def smooth_step(self, Sigma_t_t, Sigma_tpost_t, Sigma_tpost_T,
        ...
        # Smoother gain
        J_t  = torch.linalg.solve(Sigma_tpost_t.mT, A @ Sigma_t_t).mT 
        # Smoothed mean
        mu_tpost_T = mu_t_t + (J_t @ (mu_tpost_T - mu_tpost_t).T).squeeze(-1)
        # Smoothed covariance
...
if __name__ == '__main__':
    ...
    # Data (batch, time)
    n_batch, T = 16, 100
    Y, U, X = simulate_rocket_batch(n_batch, T, dt, 2.0, r_scale)
        
    dt = 0.1
    A = torch.tensor([[1., dt],[0., 1.]], dtype=torch.float32)
    B = torch.tensor([[0.5*dt*dt],[dt]], dtype=torch.float32)
    C = torch.tensor([[1., 0.]], dtype=torch.float32)

    mu0    = torch.zeros(2)
    Sigma0 = torch.diag(torch.tensor([100., 100.], dtype=torch.float32))

    kf = KalmanFilter(A, B, C, q_scale, r_scale, mu0, Sigma0)
    
    # Test
    mus_t, Sigmas_t, mus_pred, Sigmas_pred = kf.filter(Y, U)
