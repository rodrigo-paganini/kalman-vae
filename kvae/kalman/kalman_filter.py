...
class KalmanFilter(nn.Module):
    def __init__(self, q_scale, r_scale, mu0, Sigma0, dyn_params):
        super().__init__()
        ...
        # NOTE:Fixed Q,R
        self.register_buffer("Q", (q_scale**2) * torch.eye(n, dtype=dtp, device=dev))  # [n,n]
        self.register_buffer("R", (r_scale**2) * torch.eye(p, dtype=dtp, device=dev))  # [p,p]
...
if __name__ == '__main__':
    ...
    dt = 0.1
    n_batch, T = 5000, 100
    Y, U, X = simulate_rocket_batch(n_batch, T, dt, 2.0, 4.0)
    ...
    kf = KalmanFilter(q_scale, r_scale, mu0, Sigma0, dyn_params)
